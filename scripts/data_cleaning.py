...
df1 = df1.sample(560000)
df3 = df3.sample(1204000)

df1.to_csv(os.path.join(RAW_DATA_DIR, 'fraud_ds1.csv.gz'), index=False, compression='gzip')
df2.to_csv(os.path.join(RAW_DATA_DIR, 'fraud_ds2.csv.gz'), index=False, compression='gzip')
df3.to_csv(os.path.join(RAW_DATA_DIR, 'money_laundering_ds.csv.gz'), index=False, compression='gzip')
